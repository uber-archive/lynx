...
// #### @socket {Object} Optional socket if we want to share
//
// var client = new lynx('localhost', 8125);
//
// Returns a new `Lynx` client
//
function Lynx(host, port, socket) {
  //
...
Lynx.prototype.count = function (stats, delta, sample_rate) {
  ...
  //
  if (typeof delta !== 'number' && typeof delta !== 'string') {
    //
...
Lynx.prototype.send = function (stats, sample_rate) {
  var sampled_stats = Lynx.sample(stats, sample_rate)
